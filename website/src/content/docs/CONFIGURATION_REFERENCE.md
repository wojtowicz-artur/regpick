...
_Effect: `import { cn } from "@/lib/utils"` becomes `import { cn } from "~/src/utils/cn"`._
...
- `--cwd=<path>`: Changes the current working directory. Useful when running `regpick` from the root of a monorepo, targeting a specific workspace package.
  _Example:_ `npx regpick add ui --cwd=./packages/frontend`
...
- `--select=<items>`: A comma-separated list of component names to install without showing the selection prompt.
  _Example:_ `npx regpick add ui --select=button,card,dialog`
